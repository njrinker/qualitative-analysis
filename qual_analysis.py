...
# prompts = ["Give a one word sentiment analysis for each statement in a row of the following file where each row is seperated by \\r\\n: \n" + df.to_csv(index=False, header=False)]
# print(prompts)
...
for row in df.itertuples():
    print('\n', row[1])
    # Construct the list of different prompts with instructions on what the AI should do with the provided data
    prompts = ["Is this sentance positive or negative in one word: " + row[1]]
    # Create a for each loop for each individual prompt in prompts. Then, make an API call to the chat bot and save the response
    for p in prompts:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
